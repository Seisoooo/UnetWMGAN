...
def log_training_config(config):
    ...
    logging.info(f"嵌入网络学习率 (embed_learning_rate): {config['training']['embed_learning_rate']}")
    logging.info(f"提取网络学习率 (extracted_learning_rate): {config['training']['extract_learning_rate']}")
    logging.info(f"判别器学习率 (discrimnator_learning_rate): {config['training']['discriminator_learning_rate']}")
